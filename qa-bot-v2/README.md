...
- `!검증` 또는 `/검증`: 테스터, 클라이언트, 환경, 검증 항목을 선택한 뒤 기존 자동화 명령을 같은 봇 프로세스에서 실행합니다. Android APP은 기존 stg/dev 검증을 지원하고, iOS APP은 우선 게스트 로그인 stg/dev를 지원합니다. 그 외 iOS 항목과 PC, Prod는 준비 중으로 안내합니다.
