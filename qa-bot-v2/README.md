...
- `!검증` 또는 `/검증`: 테스터, 클라이언트, 환경, 검증 항목을 선택한 뒤 기존 자동화 명령을 같은 봇 프로세스에서 실행합니다. 로그인, 정확한/유연한/대화형 검색, 계약 요청/취소/승인/거절, 연장 요청/수락, 결제 기본검증과 계약 확정 취소를 선택할 수 있습니다. 현재 Android APP의 stg/dev만 실행할 수 있고 PC, iOS APP, Prod는 준비 중으로 표시됩니다.
