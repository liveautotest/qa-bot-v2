...
| `TC-CONTRACT-001` | `!게스트 계약 요청` | 정확한 일정 검색 후 숙소 계약 요청 완료까지 단독 검증 |
...
- Android와 iOS 신규 설치·낮은 버전 재설치는 선택한 역할·환경의 로그인을 이어서 실행합니다. 호스트는 업데이트 설치 후에도 호스트 로그인을 실행하며, 동일 버전 설치 생략 시에는 로그인도 생략합니다.
- PASS 결과의 버전은 설치 직후 해당 환경 앱을 다시 조회한 값이며, 이후 다른 설치 작업으로 단말 버전이 바뀔 수 있습니다.
- 결과는 테스트 채널에 Slack 요약과 대시보드 링크만 남기며 PDF 리포트는 업로드하지 않습니다.

## Slack 선택 UI

- `!검증` 또는 `/검증`: 테스터, 클라이언트, 환경, 검증 항목을 선택한 뒤 기존 자동화 명령을 같은 봇 프로세스에서 실행합니다. Android APP은 기존 stg/dev 검증을 지원하고, iOS APP은 게스트 로그인·정확한 일정 검색·유연한 일정 검색을 stg/dev에서 지원합니다. 그 외 iOS 항목과 PC, Prod는 준비 중으로 안내합니다.

### 대화형 검색 사전 구현

- `!검증`의 `대화형 검색 (준비 중)`은 개발 빌드 배포 전에 Figma 기준으로 작성한 사전 구현이며, 현재 선택 실행은 차단되어 있습니다.
- 홈의 `강남역 근처로 찾아보세요.` 추천 버튼으로 진입하며, 실제 빌드 확인 후 `src/tests/conversational-search.test.js`의 임시 좌표를 보정합니다.
- 출장, 일주일 살기, 한달살기·워케이션, 가족여행, 반려동물 동반 장기 숙박, 이사 전 장기 숙박 중 하나를 무작위로 선택해 최소 6회 대화합니다.
- 답변 관련성, 맥락 유지, 조건 변경 반영, 오류·반복 응답, 숙소 결과 노출을 자동 판정하고 XML에서 확인하기 어려운 조건은 수동 확인 필요로 결과에 남깁니다.
- 숙소 결과가 나오면 첫 번째로 보이는 집을 우선 선택해 상세로 진입합니다. 카드 클릭 영역을 찾지 못할 때만 가로 스와이프로 복구한 뒤 다시 선택합니다. 집 이름, 가격, 지역, 인원, 주요 편의시설과 대화에서 확정한 조건을 비교해 일치/불일치/수동 확인 필요로 기록합니다.
...
봇을 실행하면 QA 대시보드도 `DASHBOARD_PORT`(기본값 `4321`)에서 함께 시작됩니다. Slack을 다른 기기에서 확인한다면 `.env`의 `DASHBOARD_PUBLIC_URL`에 해당 기기에서 접근 가능한 주소를 설정합니다. `DASHBOARD_ALLOWED_CIDRS`에는 접속을 허용할 Wi-Fi 대역을 지정하며, 허용되지 않은 네트워크의 요청은 `403`으로 차단합니다.
...
- 모든 자동화 결과는 Slack 요약과 대시보드 링크로 표시하며 PDF 업로드는 생략합니다.
